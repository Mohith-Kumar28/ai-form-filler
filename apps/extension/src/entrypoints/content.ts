...
export default defineContentScript({
  ...
  main() {
    ...
    /**
     * Which field the open card belongs to, when it belongs to one.
     *
     * Needed because `focusout` defers its teardown by a frame while `focusin` runs
     * synchronously, so moving between two fields reaches `showFieldAssist` for the new field
     * with the old field's card still standing. Without knowing whose card it is, the only
     * options are "bail" — which is the bug — or "close whatever is open", which would tear
     * down the card belonging to the field being focused.
     *
     * `null` for the cards that are not anchored to a field (the launcher's own menu).
     */
    ...
    function detect(): DetectionResult | null {
      ...
      // Field ids are regenerated by every pass, so a refusal recorded against an old one
      // would land on an unrelated field.
    ...
    /**
     * Tears the card down. Deliberately says nothing about *why*.
     *
     * This used to set the dismissed flag whenever it closed an open card, which made every
     * teardown look like a refusal — accepting the suggestion, scrolling the field away, and
     * closing the previous field's card on the way to mounting the next one all counted. That
     * last one is the bug people actually hit: taking a suggestion on Full name suppressed the
     * suggestion on Email, because `mountAutofillSuggestion` calls this first.
     *
     * Refusal is now recorded by the two things that are actually refusals — the close button
     * and Escape — and recorded per field, in `dismissedSuggestions`.
     */
    ...
    /**
     * Fields whose suggestion the user has turned down, by field id.
     *
     * A set rather than a boolean because refusing the suggestion on one field says nothing
     * about the next one — and a boolean, being global, is what made accepting on Full name
     * skip Email. Cleared with the detection, whose ids it is keyed by.
     */
    ...
    function mountAutofillSuggestion(
      ...
      card = mountSuggestCard({
        ...
        onSelect: accept,
        onClose: () => {
          if (fieldId) dismissedSuggestions.add(fieldId)
          closeCard()
          dismissInputListeners()
          element.focus()
        },
    ...
    function showFieldAssist(element: HTMLElement) {
      ...
      /*
        The second half of "the suggestion only ever appears on one field".

        `focusout` defers its teardown by a frame so that focus moving into the overlay does not
        read as leaving the field. `focusin` is synchronous. So tabbing or clicking from one
        field to the next arrives here with the *previous* field's card still open, and the
        guard was a flat `if (card) return` — the new field was skipped, and a frame later the
        old card closed, leaving nothing on screen and nothing pending.

        Whose card it is, is the thing that matters. One belonging to another field is stale and
        goes; one belonging to this field is already doing its job and is left alone.
      */
      if (card) {
        if (cardFieldId === fieldId) return
        closeCard()
      }
      ...
      /*
        `hasAnswer`, not a bare emptiness test, and it is the same call the server makes.

        A phone input seeded with `+91` by its own country picker read as answered, so this took
        the branch below and returned: no sparkle, no suggestion, nothing. From the outside that
        looks like the field was never detected — it was, and then deliberately left alone.
      */
      const current = detection.adapter.readValue(detectedField)
      const answered = hasAnswer({ kind: field.kind, currentValue: current ?? undefined })
      ...
      void ensureKnownFacts().then((facts) => {
        ...
        const suggestion = facts
          ...
          : null
        /*
          No "reset the flag on the way out" here any more.

          That line existed to stop one over-broad global refusal suppressing every later field
          forever — it let the *next* field through and then re-armed. Which meant the flag
          alternated: accept on one field, lose the suggestion on the next, get it back on the
          one after. With refusal recorded per field there is nothing to unset.
        */
        if (!suggestion || dismissedSuggestions.has(fieldId)) return
    ...
    async function apply(plan: FillPlan): Promise<ApplyReport> {
      ...
      /**
       * Answers that were a lookup, not a judgement. Nothing here is worth learning back.
       *
       * Tier 0 is the router reading a value straight out of the user's own saved information —
       * their email, their phone, a fact they typed into the panel. There is no model call, and
       * the answer is a copy of something we already hold in a place we can read faster and more
       * reliably than retrieval ever will. Teaching it back writes a document into the memory
       * layer whose entire content is a fact already in the profile, spends a write against the
       * daily learning budget, and then competes with the profile at retrieval time.
       *
       * So a tier-0 field is not watched at all. Kept out of `arm` rather than filtered later,
       * because a field that is not armed also costs nothing to observe — no snapshot, no
       * settle pass, no entry to discard at the far end.
       *
       * Everything above tier 0 is still watched. An answer the model *composed* is exactly the
       * thing worth learning from, whether the user keeps it or rewrites it.
       */
      const lookups = new Set(plan.fills.filter((fill) => fill.tier === 0).map((f) => f.fieldId))

      feedback.arm(
        active.form.fields
          .filter((field) => !lookups.has(field.id))
          .map((field) => ({
            fieldId: field.id,
            label: field.label,
            kind: field.kind,
            ...
            field.options.length <= LEARN_MAX_OPTIONS
    ...
    const onFocusOut = (event: FocusEvent) => {
      ...
      requestAnimationFrame(() => {
        ...
        // A frame has passed, and `focusin` on the next field may already have replaced the
        // card with that field's own. Closing it here would undo the mount that just happened.
        if (cardFieldId !== null && target && cardFieldId !== fieldIdFor(target)) return
        closeCard()
      })
    }
